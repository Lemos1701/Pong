...
def inicial():
    ...
    while(True):
        if mouse1.is_over_area([playSolo.x, playSolo.y], [playSolo.x + playSolo.width, playSolo.y + playSolo.height]) and mouse1.is_button_pressed(1):
            click.play()
        ...
        if mouse1.is_over_area([playIA.x, playIA.y], [playIA.x + playIA.width, playIA.y + playIA.height]) and mouse1.is_button_pressed(1):
            click.play()
            timePass= time.time() - timer
            if timePass >=1:
                fundoM.stop()
        ...
        if mouse1.is_over_area([sair.x, sair.y], [sair.x + sair.width, sair.y + sair.height]) and mouse1.is_button_pressed(1):
            menuJan.close()
